	...
	implements IDatabaseCacheAdapter
{
	...
	async getMemoriesByIds(
		...
		return this.withDatabase(async () => {
			...
			const queryParams: (string | number)[] = [...memoryIds];
	...
	async getGoals(params: {
		...
		return this.withDatabase(async () => {
			...
			const values: (string | number | boolean | null)[] = [params.roomId];
			let paramCount = 1;
	...
	async searchMemoriesByEmbedding(
		...
		return this.withDatabase(async () => {
			...
			const values: (string | number | null)[] = [vectorStr, params.tableName];
	...
	async removeFileMemory(fileName: string, agentId: UUID): Promise<void> {
		return this.withDatabase(async () => {
			const query = `
                WITH source_id AS (
                    SELECT id::text
                ...
                WHERE (type = 'documents' AND content->>'text' LIKE $1 AND "agentId" = $2)
                   OR (type = 'fragments' AND content->>'source' IN (SELECT id FROM source_id));
            `;

			await this.pool.query(query, [`File name: ${fileName}%`, agentId]);
		}, "removeMemory");
	}
